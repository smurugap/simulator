...
from common.util import get_random_cidr, elem2dict, time_taken, retry
...
class ConfigApi(object):
    ...
    @time_taken
    def create_network(self, name, cidr, vlan, **kwargs):
        fq_name = self.get_fq_name(name)
...
#            pass
    ...
    @time_taken
    def create_sg(self, fq_name, rule_count=1):
        ''' Create Security group using VNC api '''
        def _get_rule(prefix, ethertype):
            dst_addr = AddressType(subnet=SubnetType(prefix, 24))
            ...
            rule = PolicyRuleType(rule_uuid=str(uuid.uuid4()), direction='>',
                                  ...
                                  dst_ports=[PortType(80, 80), PortType(22, 22)],
                                  ethertype=ethertype)
            return rule

        rules = [_get_rule('10.0.%s.0'%i, 'IPv4') for i in range(0, rule_count)]
        sg_obj = SecurityGroup(name=fq_name[-1], parent_type='project',
            fq_name=fq_name, security_group_entries=PolicyEntriesType(rules))
        try:
    ...
    @time_taken
    def delete_sg(self, fq_name):
        try:
            self.vnc.security_group_delete(fq_name=fq_name)
            print 'Deleted SG', fq_name[-1]
        except NoIdError:
            pass

    def assoc_security_group_to_vpg(self, sg_fqname, vpg_fqname):
        sg_obj = self.vnc.security_group_read(fq_name=sg_fqname)
        obj = self.vnc.virtual_port_group_read(fq_name=vpg_fqname)
        obj.add_security_group(sg_obj)
        self.vnc.virtual_port_group_update(obj)

    def disassoc_security_group_from_vpg(self, sg_fqname, vpg_fqname):
        sg_obj = self.vnc.security_group_read(fq_name=sg_fqname)
        obj = self.vnc.virtual_port_group_read(fq_name=vpg_fqname)
        obj.del_security_group(sg_obj)
        self.vnc.virtual_port_group_update(obj)

    def assoc_port_profile_to_vpg(self, pp_fqname, vpg_fqname):
        pp_obj = self.vnc.port_profile_read(fq_name=pp_fqname)
        obj = self.vnc.virtual_port_group_read(fq_name=vpg_fqname)
        obj.add_port_profile(pp_obj)
        return self.vnc.virtual_port_group_update(obj)

    def disassoc_port_profile_from_vpg(self, pp_fqname, vpg_fqname):
        pp_obj = self.vnc.port_profile_read(fq_name=pp_fqname)
        obj = self.vnc.virtual_port_group_read(fq_name=vpg_fqname)
    ...
    def create_storm_control_profile(self, fq_name, action=None,
        recovery_timeout=None, bandwidth=10, no_broadcast=False,
